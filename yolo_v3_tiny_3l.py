...
def yolo_v3_tiny_3l(inputs, num_classes, is_training=False, data_format='NCHW', reuse=False):
    """
        ...
        with slim.arg_scope([slim.conv2d, slim.batch_norm, _fixed_padding], reuse=reuse):
            with slim.arg_scope([slim.conv2d],
                ...
                with tf.variable_scope('yolo-v3-tiny'):
                    ...
                    # Third output
                    inputs = _conv2d_fixed_padding(route_4, 128, 1)
                    upsample_size = route_3.get_shape().as_list()
                    inputs = _upsample(inputs, upsample_size, data_format)
                    inputs = tf.concat([inputs, route_3], axis=1 if data_format == 'NCHW' else 3)
